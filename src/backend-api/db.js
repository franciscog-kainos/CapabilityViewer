...
//Get details for trainings
exports.getTrainings = function (callback) {
    db.query('SELECT Training.* FROM capabilitiesDB_test.Training',
        function (err, rows) {
            if (err) return callback(err, null);
            callback(err, rows);
        });
...
//Get details for training categories
exports.getTrainingCategories = function (callback) {
    db.query('SELECT Training_Category.* FROM capabilitiesDB_test.Training_Category',
        function (err, rows) {
            if (err) return callback(err, null);
            callback(err, rows);
        });
...
//Get details for band training and training categories
exports.getTrainingResources = function (callback) {
    db.query('SELECT Training.training_name, Training_Category.training_category_name ' +
        'FROM capabilitiesDB_test.Training, capabilitiesDB_test.Training_Category ' +
        'WHERE Training.training_category_id = Training_Category.training_category_id;',
        function (err, rows) {
            if (err) return callback(err, null);
            callback(err, rows);
        });
...
exports.insertBand = function (data, callback) {
    db.query(
        'INSERT INTO Band SET ? ', data, function (err, results, fields) {
            console.log(data);
            if (err) return callback(err, null);
...
//Add new band training relations
exports.insertBandTraining = function (data, callback) {
    console.log(data)
    db.query(
        'INSERT INTO Band_Training SET ? ', data[i], function (err, results, fields) {
            console.log(data);
            if (err) return callback(err, null);
            callback(err, results.insertId);
        });
